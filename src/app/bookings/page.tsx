...
export default function BookingsAdminPage() {
  ...
  const { data: bookingsData, isLoading: bookingsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(`/bookings${status ? `?status=${status}` : ""}`);
      // Backend returns { bookings: [...], total: N }
      // Guard against legacy plain-array shape just in case
      if (Array.isArray(res.data)) return { bookings: res.data as Booking[], total: (res.data as Booking[]).length };
      return res.data as { bookings: Booking[]; total: number };
    },
  });
  ...
  const createSlotMutation = useMutation({
    mutationFn: async (dto: object) => (await api.post("/bookings/slots", dto)).data,
    onSuccess: () => { toast.success("Slot created!"); queryClient.invalidateQueries({ queryKey: ["admin-slots"] }); },
    onError: (err: any) => toast.error(err.message),
  });

  const deleteSlotMutation = useMutation({
    mutationFn: async (id: string) => (await api.delete(`/bookings/slots/${id}`)).data,
    onSuccess: () => { toast.success("Slot deleted"); queryClient.invalidateQueries({ queryKey: ["admin-slots"] }); },
    onError: (err: any) => toast.error(err.message),
  });

  const approveMutation = useMutation({
    mutationFn: async ({ id, feeAmount }: { id: string; feeAmount: number }) =>
      (await api.patch(`/bookings/${id}/approve`, { consultationFee: feeAmount })).data,
    onSuccess: () => {
      toast.success("Booking approved! Quote email sent.");
      queryClient.invalidateQueries({ queryKey: ["admin-bookings"] });
      setSelectedBooking(null); setFee("");
    },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  const handleCreateSingleSlot = () => {
    if (!slotDate) return toast.error("Please select a date");
    createSlotMutation.mutate({ date: slotDate, startTime: slotStart, endTime: slotEnd, sessionType: slotType });
  };

  const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  const DAY_JS_INDEX: Record<string, number> = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
  ...
  const handleCreateBulkSlots = async () => {
    ...
    for (let w = 0; w < bulkWeeks; w++) {
      for (const day of bulkDays) {
        const d = new Date(today);
        let offset = DAY_JS_INDEX[day] - today.getDay();
        if (offset < 0) offset += 7;
        offset += w * 7;
        d.setDate(today.getDate() + offset);
        promises.push(api.post("/bookings/slots", {
          date: d.toISOString().split("T")[0],
          startTime: slotStart, endTime: slotEnd, sessionType: slotType,
        }));
      }
    }
    try {
      ...
      toast.success(`Created ${promises.length} slots!`);
      queryClient.invalidateQueries({ queryKey: ["admin-slots"] });
      setShowSlotForm(false); setBulkDays([]);
        ...
        {(["bookings", "slots"] as const).map((tab) => (
          ...
          >
            {tab === "bookings"
              ...
              : `Available Slots${slots.length ? ` (${slots.length})` : ""}`}
          </button>
        ))}
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    {["Client", "Session", "Date & Time", "Notes", "Status", "Submitted", "Actions"].map((h) => (
                      <th key={h} className="text-left font-body text-xs font-bold text-gray-400 uppercase tracking-wider py-4 px-6">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    Array.from({ length: 5 }).map((_, i) => (
                      <tr key={i} className="border-b border-gray-50">
                        {Array.from({ length: 7 }).map((_, j) => (
                          <td key={j} className="py-4 px-6"><div className="h-4 bg-gray-100 rounded animate-pulse w-3/4" /></td>
                        ))}
                      </tr>
                    ))
                  ) : bookings.length === 0 ? (
                    ...
                    bookings.map((b) => (
                      <tr key={b._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="py-4 px-6 max-w-[160px]">
                          <p className="font-body text-xs text-gray-500 truncate">{b.notes || "—"}</p>
                        </td>
                        <td className="py-4 px-6">
                          <span className={`font-body text-xs font-bold px-3 py-1.5 rounded-full capitalize ${statusColor[b.status]}`}>
                            {b.status}
                        ...
                        <td className="py-4 px-6">
                          {b.status === "pending" && (
                            <button
                              onClick={() => { setSelectedBooking(b); setFee(""); setRejectReason(""); }}
                              className="font-body text-xs font-semibold text-brand-purple hover:text-brand-purple-light transition-colors flex items-center gap-1"
                            >
                              Review <ChevronDown size={12} />
                            </button>
        ...
        <>
          {slotsLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center shadow-sm">
              <Calendar className="text-gray-200 mx-auto mb-4" size={48} />
              <h3 className="font-display font-bold text-gray-900 text-lg mb-2">No slots yet</h3>
              <p className="font-body text-gray-400 text-sm mb-6">Create available slots so clients can book consultations.</p>
              <button onClick={() => setShowSlotForm(true)} className="btn-primary inline-flex items-center gap-2 text-sm">
                <Plus size={14} /> Create First Slot
              </button>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {slots.map((slot) => (
                <motion.div key={slot._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-start justify-between group"
                >
                  <div>
                    <p className="font-body font-semibold text-gray-800 text-sm mb-1">{slot.sessionType}</p>
                    <p className="font-display font-bold text-gray-900">{format(new Date(slot.date), "EEE d MMM yyyy")}</p>
                    <p className="font-body text-sm text-gray-500 mt-1 flex items-center gap-1">
                      <Clock size={12} /> {slot.startTime} — {slot.endTime}
                    </p>
                    <span className="inline-block mt-2 font-body text-xs font-bold px-2.5 py-1 rounded-full bg-brand-mint/10 text-brand-teal">Available</span>
                  </div>
                  <button
                    onClick={() => { if (confirm("Delete this slot?")) deleteSlotMutation.mutate(slot._id); }}
                    className="p-2 rounded-xl text-gray-300 hover:text-red-400 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100"
        ...
        {showSlotForm && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-display text-xl font-bold text-gray-900">Create Available Slots</h3>
                <button onClick={() => setShowSlotForm(false)} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400"><X size={18} /></button>
              </div>

              <div className="flex flex-col gap-4 mb-6">
                <div>
                  <label className="font-body text-sm font-semibold text-gray-700 block mb-2">Session Type</label>
                  <select value={slotType} onChange={(e) => setSlotType(e.target.value)} className="input-base">
                    {SESSION_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="font-body text-sm font-semibold text-gray-700 block mb-2">Start Time</label>
                    <input type="time" value={slotStart} onChange={(e) => setSlotStart(e.target.value)} className="input-base" />
                  </div>
                  <div>
                    <label className="font-body text-sm font-semibold text-gray-700 block mb-2">End Time</label>
                    <input type="time" value={slotEnd} onChange={(e) => setSlotEnd(e.target.value)} className="input-base" />
                  </div>
                </div>
              </div>
              ...
              {/* Single slot */}
              <div className="border border-gray-100 rounded-2xl p-5 mb-4">
                <p className="font-body text-sm font-semibold text-gray-700 mb-3">Single Slot</p>
                <input type="date" value={slotDate} min={new Date().toISOString().split("T")[0]}
                  onChange={(e) => setSlotDate(e.target.value)} className="input-base mb-3" />
                <button onClick={handleCreateSingleSlot} disabled={createSlotMutation.isPending || !slotDate}
                  className="btn-primary w-full text-sm justify-center flex items-center gap-2 disabled:opacity-50"
                >
                  <Plus size={14} /> {createSlotMutation.isPending ? "Creating..." : "Add Single Slot"}
                </button>
              </div>

              {/* Recurring slots */}
              <div className="border border-gray-100 rounded-2xl p-5">
                <p className="font-body text-sm font-semibold text-gray-700 mb-1">Recurring Slots</p>
                <p className="font-body text-xs text-gray-400 mb-3">Pick days of the week and how many weeks ahead</p>
                <div className="flex gap-2 flex-wrap mb-4">
                  {DAYS.map((d) => (
                    <button key={d} type="button" onClick={() => toggleBulkDay(d)}
                      className={`font-body text-xs font-semibold w-11 h-11 rounded-xl transition-all ${
                        bulkDays.includes(d) ? "bg-brand-gradient text-white shadow-md" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                      }`}
                    >{d}</button>
                  ))}
                </div>
                <div className="flex items-center gap-3 mb-4">
                  <label className="font-body text-sm text-gray-600 shrink-0">Weeks ahead:</label>
                  <input type="number" min={1} max={12} value={bulkWeeks}
                    onChange={(e) => setBulkWeeks(Number(e.target.value))} className="input-base w-24" />
                  <span className="font-body text-xs text-gray-400">
                    = {bulkDays.length * bulkWeeks} slot{bulkDays.length * bulkWeeks !== 1 ? "s" : ""}
                  </span>
                </div>
                <button onClick={handleCreateBulkSlots} disabled={bulkDays.length === 0}
                  className="btn-primary w-full text-sm justify-center flex items-center gap-2 disabled:opacity-50"
                >
                  <CalendarPlus size={14} /> Create {bulkDays.length * bulkWeeks || 0} Recurring Slots
                </button>
              </div>
            </motion.div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                ...
                <input type="number" value={fee} onChange={(e) => setFee(e.target.value)} placeholder="e.g. 50" className="input-base" />
                <button
                  disabled={!fee || approveMutation.isPending}
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)}
                  placeholder="Explain why this booking is being rejected..." rows={3} className="input-base resize-none text-sm" />
                <button
                  disabled={!rejectReason || rejectMutation.isPending}
              ...
              <button onClick={() => setSelectedBooking(null)} className="mt-4 w-full font-body text-sm text-gray-400 hover:text-gray-600 transition-colors">
                Cancel
              </button>
            </motion.div>
          </motion.div>
